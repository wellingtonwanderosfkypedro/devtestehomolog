...
const schema = yup.object().shape({
  company: yup.string().min(3, 'Mínimo de 3 caracteres').required('Campo obrigatório*'),
  area: yup.string().required('Campo obrigatório*'),
  office: yup.string().required('Campo obrigatório*'),
  corporateEmail: yup.string().email('E-mail inválido').required('Campo obrigatório*'),
  demand: yup.string().min(10, 'Mínimo de 10 caracteres').required('Campo obrigatório*'),
...
const Form: React.FC = () => {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    //enviar dados
    const endpointURL = "https://cloud.conteudo.neoway.com.br/handler-contato-comercial";
        ...
        <form onSubmit={handleSubmit(onSubmit)} >
          ...
          {currentStep === 2 && (
            <>
              ...
              />
              <Button
                message='Enviar ao time comercial'
                type='submit'
              />
            </>
          )}
